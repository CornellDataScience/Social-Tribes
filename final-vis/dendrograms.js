...
function display(data){
  ...
  var people = data.people.map(function(x){return " "+ names[x]});
  if (people.length > 50){
    people = people.slice(0,50);
    people.push("...");
  }
  document.getElementById("infoTop").innerHTML = "SIZE OF CLUSTER: "+size.toString();
  document.getElementById("infoBottom").innerHTML = "PEOPLE IN CLUSTER:" + people.toString();
}
...
var selectHelper = function(d){
  ...
  if (d.children){
    for (var i = 0; i<d.children.length; i++){
      ...
      d3.select("#"+id2).attr("fill","maroon");
    }              
  }
  display(d["data"]["data"]);          
}
